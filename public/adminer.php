...
password_file($Ab){$Hc=get_temp_dir()."/adminer.key";$J=@file_get_contents($Hc);if($J||!$Ab)return$J;$Rc=@fopen($Hc,"w");if($Rc){chmod($Hc,0660);$J=rand_string();fwrite($Rc,$J);fclose($Rc);}return$J;}function
rand_string(){return
...
false;}return
true;}function
alter_indexes($Q,$c){$Ab=array();$Xb=array();$Jf=array();foreach($c
as$X){if($X[0]!="INDEX")$Ab[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$Xb[]=idf_escape($X[1]);else$Jf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($Q."_"))." ON ".table($Q)." (".implode(", ",$X[2]).")";}if($Ab)array_unshift($Jf,"ALTER TABLE ".table($Q).implode(",",$Ab));if($Xb)array_unshift($Jf,"DROP INDEX ".implode(", ",$Xb));foreach($Jf
as$H){if(!queries($H))return
false;}return
true;}function
truncate_tables($S){return
...
last_id(){}function
hmac($Aa,$Eb,$x,$Of=false){$Ta=64;if(strlen($x)>$Ta)$x=pack("H*",$Aa($x));$x=str_pad($x,$Ta,"\0");$Fd=$x^str_repeat("\x36",$Ta);$Gd=$x^str_repeat("\x5C",$Ta);$J=$Aa($Gd.pack("H*",$Aa($Fd.$Eb)));if($Of)$J=pack("H*",$J);return$J;}function
...
extends
Min_SQL{function
select($Q,$M,$Z,$Wc,$Ue=array(),$z=1,$E=0,$Bf=false){global$b;$Eb=array();$H="$Q/_search";if($M!=array("*"))$Eb["fields"]=$M;if($Ue){$_g=array();foreach($Ue
as$kb){$kb=preg_replace('~ DESC$~','',$kb,1,$_b);$_g[]=($_b?array($kb=>"desc"):$kb);}$Eb["sort"]=$_g;}if($z){$Eb["size"]=+$z;if($E)$Eb["from"]=($E*$z);}foreach($Z
as$X){list($kb,$Ne,$X)=explode(" ",$X,3);if($kb=="_id")$Eb["query"]["ids"]["values"][]=$X;elseif($kb.$X!=""){$bh=array("term"=>array(($kb!=""?$kb:"_all")=>$X));if($Ne=="=")$Eb["query"]["filtered"]["filter"]["and"][]=$bh;else$Eb["query"]["filtered"]["query"]["bool"]["must"][]=$bh;}}if($Eb["query"]&&!$Eb["query"]["filtered"]["query"]&&!$Eb["query"]["ids"])$Eb["query"]["filtered"]["query"]=array("match_all"=>array());$Eg=microtime(true);$ng=$this->_conn->query($H,$Eb);if($Bf)echo$b->selectQuery("$H: ".print_r($Eb,true),format_time($Eg));if(!$ng)return
...
set_charset($Za){if(function_exists('mysql_set_charset'))return
mysql_set_charset($Za,$this->_link);return$this->query("SET NAMES $Za");}function
...
idf_escape($t);}function
connect(){global$b;$g=new
Min_DB;$i=$b->credentials();if($g->connect($i[0],$i[1],$i[2])){$g->set_charset(charset($g));$g->query("SET sql_quote_show_create = 1, autocommit = 1");return$g;}$J=$g->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($jg=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$jg;return$J;}function
...
limit1($H,$Z){return
limit($H,$Z,1);}function
db_collation($l,$mb){global$g;$J=null;$Ab=$g->result("SHOW CREATE DATABASE ".idf_escape($l),1);if(preg_match('~ COLLATE ([^ ]+)~',$Ab,$B))$J=$B[1];elseif(preg_match('~ CHARACTER SET ([^ ]+)~',$Ab,$B))$J=$mb[$B[1]][-1];return$J;}function
...
foreign_keys($Q){global$g,$Ke;static$qf='`(?:[^`]|``)+`';$J=array();$Bb=$g->result("SHOW CREATE TABLE ".table($Q),1);if($Bb){preg_match_all("~CONSTRAINT ($qf) FOREIGN KEY ?\\(((?:$qf,? ?)+)\\) REFERENCES ($qf)(?:\\.($qf))? \\(((?:$qf,? ?)+)\\)(?: ON DELETE ($Ke))?(?: ON UPDATE ($Ke))?~",$Bb,$be,PREG_SET_ORDER);foreach($be
...
routine($C,$U){global$g,$oc,$sd,$zh;$Ba=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$yh="((".implode("|",array_merge(array_keys($zh),$Ba)).")\\b(?:\\s*\\(((?:[^'\")]|$oc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$qf="\\s*(".($U=="FUNCTION"?"":$sd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$yh";$Ab=$g->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$qf\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$yh\\s+":"")."(.*)~is",$Ab,$B);$o=array();preg_match_all("~$qf\\s*,?~is",$B[1],$be,PREG_SET_ORDER);foreach($be
...
permanentLogin($Ab=false){return
password_file($Ab);}function
...
dumpTable($Q,$Jg,$Bd=0){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($Jg)dump_csv(array_keys(fields($Q)));}elseif($Jg){if($Bd==2){$o=array();foreach(fields($Q)as$C=>$n)$o[]=idf_escape($C)." $n[full_type]";$Ab="CREATE TABLE ".table($Q)." (".implode(", ",$o).")";}else$Ab=create_sql($Q,$_POST["auto_increment"]);if($Ab){if($Jg=="DROP+CREATE"||$Bd==1)echo"DROP ".($Bd==2?"VIEW":"TABLE")." IF EXISTS ".table($Q).";\n";if($Bd==1)$Ab=remove_definer($Ab);echo"$Ab;\n\n";}}}function
...
',(support("comment")?"<td".($rb?"":" class='hidden'").">".'Comment':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.0' alt='+' title='".'Add next'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($rb?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.0' alt='+' title='".'Add next'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.2.0' alt='^' title='".'Move up'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.2.0' alt='v' title='".'Move down'."'>&nbsp;":""),($Ye==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.0' alt='x' title='".'Remove'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
drop_create($Xb,$Ab,$Yb,$ch,$ac,$A,$me,$ke,$le,$Ge,$xe){if($_POST["drop"])query_redirect($Xb,$A,$me);elseif($Ge=="")query_redirect($Ab,$A,$le);elseif($Ge!=$xe){$Cb=queries($Ab);queries_redirect($A,$ke,$Cb&&queries($Xb));if($Cb)queries($Yb);}else
queries_redirect($A,$ke,queries($ch)&&queries($ac)&&queries($Xb)&&queries($Ab));}function
...
as$Yg)echo"\n<div class='references' title='".h($Zg)."' id='refd$Pd-".($r++)."' style='left: $Qd"."em; top: ".$Q["fields"][$Yg]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.2.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Qd)."em;'></div></div>";}}echo"\n</div>\n";}foreach($lg
...
as$l){$b->dumpDatabase($l);if($g->select_db($l)){if($_d&&preg_match('~CREATE~',$Jg)&&($Ab=$g->result("SHOW CREATE DATABASE ".idf_escape($l),1))){if($Jg=="DROP+CREATE")echo"DROP DATABASE IF EXISTS ".idf_escape($l).";\n";echo"$Ab;\n";}if($_d){if($Jg)echo
use_sql($l).";\n\n";$df="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$fg){foreach(get_rows("SHOW $fg STATUS WHERE Db = ".q($l),null,"-- ")as$K)$df.=($Jg!='DROP+CREATE'?"DROP $fg IF EXISTS ".idf_escape($K["Name"]).";;\n":"").remove_definer($g->result("SHOW CREATE $fg ".idf_escape($K["Name"]),2)).";;\n\n";}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K)$df.=($Jg!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"").remove_definer($g->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3)).";;\n\n";}if($df)echo"DELIMITER ;;\n\n$df"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$Vh=array();foreach(table_status('',true)as$C=>$R){$Q=(DB==""||in_array($C,(array)$_POST["tables"]));$Eb=(DB==""||in_array($C,(array)$_POST["data"]));if($Q||$Eb){if($_c=="tar"){$mh=new
TmpFile;ob_start(array($mh,'write'),1e5);}$b->dumpTable($C,($Q?$_POST["table_style"]:""),(is_view($R)?2:0));if(is_view($R))$Vh[]=$C;elseif($Eb){$o=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($o,$o)." FROM ".table($C));}if($_d&&$_POST["triggers"]&&$Q&&($wh=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$wh\nDELIMITER ;\n";if($_c=="tar"){ob_end_flush();tar_file((DB!=""?"":"$l/")."$C.csv",$mh);}elseif($_d)echo"\n";}}foreach($Vh
...
<input type="submit" value="Save">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Drop'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.0' alt='+' title='".'Add next'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE")$Vc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Vc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$He=$B[1];}}if($_POST&&!$m){$Ie=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Ie",ME."privileges=",'User has been dropped.');else{$ye=q($_POST["user"])."@".q($_POST["host"]);$nf=$_POST["pass"];if($nf!=''&&!$_POST["hashed"]){$nf=$g->result("SELECT PASSWORD(".q($nf).")");$m=!$nf;}$Cb=false;if(!$m){if($Ie!=$ye){$Cb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ye IDENTIFIED BY PASSWORD ".q($nf));$m=!$Cb;}elseif($nf!=$He)queries("SET PASSWORD FOR $ye = ".q($nf));}if(!$m){$cg=array();foreach($we
